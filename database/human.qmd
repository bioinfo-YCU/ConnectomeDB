        ...
        buttons: [
                ...
                exportOptions: {
                    ...
                    format: {
                        body: function(data, row, column, node) {
                            // For original column 10 (the URL column), extract the href attribute
                            // In exportOptions format.body, `column` is the *visible* column index.
                            // If original column 10 is at internal index 11 (after checkbox),
                            // and if it's visible, then `column` will be its visible index.
                            // Assuming original column 10 is visible and is the 11th visible column (index 10).
                            // This `column` index is relative to the `columns` selected for export.
                            // If you are excluding the first column (:not(:first-child)), then the
                            // effective index for the original column 10 would be 9 (10 - 1).
                            // Let's use the full DataTables internal index for robustness if possible,
                            // or verify the `column` parameter's meaning carefully.
                            // For simplicity, let's assume `column` here refers to the index within the exported columns.
                            // If original column 10 is at DataTables index 11, and we export all but first,
                            // then it will be at export index 10.
                            if (column === 10) { // This refers to the visible column index for export (original column 11 if checkbox is removed)
            ...
            },
            {
                ...
                exportOptions: {
                    ...
                    format: {
                        body: function(data, row, column, node) {
                            // Similar logic for column index in exportOptions
                            if (column === 10) { // Assuming original column 10 is at export index 9
                                var $temp = $('<div>').html(data);
                                var href = $temp.find('a').attr('href');
                                return href ? encodeURI(href) : data;
                            }
