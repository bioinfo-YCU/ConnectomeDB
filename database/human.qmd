---
title: "{{< fa database >}} ConnectomeDB2025: <span class='highlight'>Human LR Pairs</span>"
execute:
  echo: false
  warnings: false 
format:
  html:
    table: false
header-includes: |
  <script src="../js/keepDropdownMenuGold.js"></script>
   <script>
    document.addEventListener("DOMContentLoaded", function () {
      const columnNames = ["Interaction ID", "Human LR Pair", "Ligand", 
                                   "Ligand symbol and aliases", "Ligand HGNC ID",
                                   "Ligand location", "Receptor", 
                                   "Receptor symbol and aliases", "Receptor HGNC ID",
                                   "Receptor location", "PMID", "Perplexity",
                                   "Database Source", "binding location",
                                   "bind in trans?", "bidirectional signalling?",
                                   "interaction type", "KEGG Pathway", "Cancer-related",
                                   "Disease Type"];
    
        function addFilterLabels() {
          const allThs = Array.from(document.querySelectorAll("thead tr th"));
          allThs.forEach((th, idx) => {
            // Remove any existing label first
            const existingLabel = th.querySelector(".filter-label");
            if (existingLabel) {
              existingLabel.remove();
            }
        
            const input = th.querySelector("input");
            if (input) {
              const label = document.createElement("div");
              label.className = "filter-label";
              label.textContent = columnNames[idx] || `Column ${idx + 1}`;
              label.style.fontWeight = "bold";
              label.style.fontSize = "0.85em";
              label.style.marginBottom = "2px";
              input.parentNode.insertBefore(label, input);
            }
          });
      }
    
      // Initial run
      setTimeout(addFilterLabels, 200);
    
      // Also rerun every time thead changes (like columns becoming visible)
      const observer = new MutationObserver(() => {
        addFilterLabels();
      });
    
      const thead = document.querySelector("thead");
      if (thead) {
        observer.observe(thead, {
          childList: true,
          subtree: true,
        });
      }
    
      // Fix empty labels in column visibility dropdown
      document.addEventListener("click", function () {
        const intervalId = setInterval(() => {
          const buttons = document.querySelectorAll(".dt-button-collection button.dt-button");
    
          if (buttons.length > 0) {
            buttons.forEach((btn, idx) => {
              const span = btn.querySelector("span");
              const isEmpty = btn.textContent.trim() === "";
              if (span && isEmpty) {
                span.textContent = columnNames[idx] || `Column ${idx + 1}`;
                span.style.color = "black"; // Ensure it's visible
              }
            });
            clearInterval(intervalId);
          }
        }, 100);
        setTimeout(() => clearInterval(intervalId), 3000); // Safety timeout
      });
    });
  </script>




---

***

<!-- ## Species (if we have relaible evidence from PMID) | Ligand Symbol & Aliases | Receptor Symbol & Aliases are the three items than can be shown on top of the DB - no more than 1 line -->

```{=html}

<style>

/* Ensure column visibility dropdown text is readable */
div.dt-button-collection {
  background-color: white !important;
  color: black !important;
  font-size: 14px !important;
  font-weight: normal !important;
  padding: 8px !important;
  border-radius: 6px !important;
  box-shadow: 0 4px 10px rgba(0,0,0,0.2);
}

div.dt-button-collection .dt-button {
  background-color: white !important;
  color: black !important;
  text-align: left !important;
  width: 100% !important;
  font-size: 13px !important;
  font-weight: normal !important;
  display: flex;
  align-items: center;
  justify-content: start;
}

/* Target the first search pane */
.dtsp-searchPane:first-child::before {
    content: "Ligand Official Symbol & Aliases";
    font-weight: bold;
}

/* Target the second search pane */
.dtsp-searchPane:nth-child(2)::before {
    content: "Receptor Official Symbol & Aliases";
    font-weight: bold;
}

/* Target the third search pane */
.dtsp-searchPane:nth-child(3)::before {
    content: "Database source (temp)";
    font-weight: bold;
}

/* Make dropdown text visible without breaking table headers */
div.dt-button-collection .dt-button {
  color: black !important;
  background-color: white !important;
  font-weight: normal !important;
  text-align: left !important;
}


</style>


```


```{python}
import sys
import re
import os
import pandas as pd
import warnings
from pandas.errors import SettingWithCopyWarning

# Suppress the SettingWithCopyWarning
warnings.simplefilter(action='ignore', category=SettingWithCopyWarning)
warnings.filterwarnings("ignore", category=pd.errors.DtypeWarning)
from itables import init_notebook_mode, show
from IPython.display import display, Javascript, HTML
import itables.options as opt
# Change working directory to ConnectomeDB
project_root = os.path.dirname(os.getcwd())
os.chdir(project_root)
sys.path.append(os.path.abspath("src"))
from createDataTable import gene_pair

human_gene_pair = gene_pair.iloc[:, :-40]
human_gene_pair.columns
def clean_column_name(col):
    # Match text between '>' and '</span>', ignoring any optional `&nbsp;`
    match = re.search(r'>(.*?)<\/span>', col)
    if match:
        return match.group(1).replace('&nbsp;', '').strip()
    return col  # Return original if no match

# Apply to all column names
human_gene_pair.columns = [clean_column_name(col) for col in human_gene_pair.columns]


human_gene_pair = human_gene_pair[["Interaction ID", "Human LR Pair", "Ligand", 
                                   "Ligand symbol and aliases", "Ligand HGNC ID",
                                   "Ligand location", "Receptor", 
                                   "Receptor symbol and aliases", "Receptor HGNC ID",
                                   "Receptor location", "PMID", "Perplexity",
                                   "Database Source", "binding location",
                                   "bind in trans?", "bidirectional signalling?",
                                   "interaction type", "KEGG Pathway", "Cancer-related",
                                   "Disease Type"]]

# Turn Interaction ID into clickable links
human_gene_pair[human_gene_pair.columns[0]] = human_gene_pair[human_gene_pair.columns[0]].apply(
    lambda x: f"<a href='https://comp.med.yokohama-cu.ac.jp/collab/connectomeDB/database/filter/{x}.html'>{x}</a>"
)

# Function to update and display the table
def update_table():
    show(
        human_gene_pair,
        keys=True,
        layout={"topStart": "info", "topEnd": "paging",
                "bottomStart": "buttons", "bottomEnd": "search"},
        fixedColumns={"start": 2, "end": 0},
        fixedHeader=True,  # Keep header fixed for alignment
        column_filters="header", 
        scrollX=True,
        classes="display nowrap cell-border compact",
        select=True,
        selected_rows=[],
        responsive=False,  # Enable responsive behavior
        theme='grid',
        search={"regex": True, "caseInsensitive": True, "smart": True},
        lengthMenu=[ [10, 20, 50, 100, -1], ["10", "20", "50", "100", "All"] ],
        buttons=[
            "pageLength",  
            {"extend": "colvis", "text": "Display Columns"},
            {"extend": "csvHtml5", "title": "Source"},
            {"extend": "excelHtml5", "title": "Source"},
            {"extend": "copyHtml5", "title": "Source"},
        ],
        columnDefs=[
            {"className": "dt-center", "targets": [11]}, 
            {"search": False, "targets": [11]},
            #{"width": "150px", "targets": [0,1]},
            #{"visible": False, "targets": [2,7,8,9,10,15,16,17]} 
            #{"visible": False, "targets": list(range(-1, -9, -1))} # Hide last 8 columns by default
        ],
        style="width:100%; max-width:1200px; margin:auto",
           language={
            "search": "Search Any Column:",
            "searchPlaceholder": "e.g. CD80",
            "searchPanesPlaceholder": ""
            }, 
        autoWidth=True,
        maxBytes=0,  # So it does not downsample
        initCode='''
    $(document).on('init.dt', function () {
        document.dispatchEvent(new Event('tableReady'));
    });
        ''',

)

# Call the function to render the table
update_table()
```

{{< include hint.qmd >}}



