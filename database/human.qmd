---
title: "Human – *Homo sapiens* (2025-09-18) </span>"
execute:
  echo: false
format:
  html:
...
<div id="loading-widget" class="loading-container">
  ...
  <div class="loading-text">Loading human interactions database...</div>
  <div class="loading-subtext">This may take a few moments</div>
</div>

<div id="table-content" style="opacity: 0;">
...
$.getJSON("../JSON/human_gene_pair.json", function(data) {
  ...
  {
    "data": "Evidence",
    "title": "<span title=\"Direct: experimentally verified; Conservation: inferred from orthology\">Evidence</span>"
  },
  {
    "data": "A_I_summary",
  ...
  {
    "data": "Ligand",
    "title": "<span title=\"HGNC gene symbol for the ligand\">Ligand</span>"
  },
  {
    "data": "Receptor",
    "title": "<span title=\"HGNC gene symbol for the receptor\">Receptor</span>"
  },
  {
    "data": "Ligand_HGNC_ID",
    "title": "<span title=\"HGNC gene ID for the ligand (link to HGNC)\">Ligand HGNC ID</span>"
  },
  {
    "data": "Receptor_HGNC_ID",
    "title": "<span title=\"HGNC gene ID for the receptor (link to HGNC)\">Receptor HGNC ID</span>"
  },
  {
  ...
  },
  {
    ...
    var hiddenColumns = []; // Hide original columns 3 and 4

    // // NO NEED for now --- Add last 2 columns to hidden list (adjust this number as needed)
    ...
    var table = $('#human-table').DataTable({
                ...
                exportOptions: {
                    ...
                    format: {
                        body: function(data, row, column, node) {
                            // Detect A.I. summary column by the chat bubble icon &#128172; instead of column position
        ...
        initComplete: function () {
            ...
            setTimeout(function() {
                ...
                var input = $('<input type="text" placeholder="Search Ligand(s) / Receptor(s): i.e. ACE, BK-2, CD24, EGFR, kuz" />') // Updated placeholder
                    .css({
                        'width': '100%',
                        'padding': '8px',
                        'font-size': '1.1rem',
                    ...
                    .on('input keyup', function () {
                        ...
                        if (val === '') {
                            ...
                            if (searchTerms.length > 0) {
                                // Use safe column names to search in Ligand_Symbols and Receptor_Symbols columns
                                $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
                                    // Get the full row data object to access by safe column name
                                    var rowData = table.row(dataIndex).data();
                                    
                                    var ligandSymbols = rowData['Ligand_Symbols'] ? rowData['Ligand_Symbols'].toString().replace(/<[^>]*>/g, '').toLowerCase() : '';
                                    var receptorSymbols = rowData['Receptor_Symbols'] ? rowData['Receptor_Symbols'].toString().replace(/<[^>]*>/g, '').toLowerCase() : '';
                                    
                                    console.log("Searching for:", searchTerms);
                                    console.log("Ligand Symbols:", ligandSymbols);
                                    console.log("Receptor Symbols:", receptorSymbols);

                                    return searchTerms.some(term =>
                                        ligandSymbols.includes(term) || receptorSymbols.includes(term)
                                    );
                                });
    ...
    // FIXED: Custom sort function to use column indices instead of names
    api.on('order.dt search.dt', function () {
        ...
        if (searchVal && searchVal.trim() !== '') {
            ...
            if (currentOrder.length > 0) {
                ...
                // If sorting by a column other than our search columns (Ligand_Symbols or Receptor_Symbols), 
            ...
            if (isCustomSearchActive) {
                api.rows().sort(function (rowIdxA, rowIdxB) {
                    ...
                    // Use safe column names to access data
                    const valuesA = [normalize(rowDataA['Ligand_Symbols']), normalize(rowDataA['Receptor_Symbols'])];
                    const valuesB = [normalize(rowDataB['Ligand_Symbols']), normalize(rowDataB['Receptor_Symbols'])];
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Fallback: if nothing happens after 5 sec, show the table anyway
    setTimeout(function() {
        if (document.getElementById('table-content').style.opacity == '0') {
