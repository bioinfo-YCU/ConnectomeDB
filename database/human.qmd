...
header-includes: |
  ...
  <script>
    // Custom JavaScript to collapse all search panes by default
    $(document).on('draw.dt', function () {
        // Collapse all the search panes by default
        $(".searchPanes").each(function() {
            $(this).find(".dtsp-searchPane").each(function() {
                if ($(this).hasClass('dtsp-open')) {
                    $(this).find(".dtsp-paneHeader").trigger("click");  // Trigger collapse
                }
            });
        });
    });
...
def update_table():
    show(
        ...
        searchPanes={"layout": "rows-1", "cascadePanes": True, "columns": [1,2,3,8,9,10,11,12], "regex": True, "caseInsensitive": False, "smart": True, "initCollapsed": True},
