...
<div id="table-content-zebrafish" style="opacity: 0;">
...
<table id="zebrafish-table" class="display nowrap cell-border compact" style="width:100%">
    <thead></thead>
    <tfoot></tfoot>
</table>
...
$(document).ready(function() {
    ...
    $.getJSON("../../JSON/zebrafish_gene_pair.json", function(data) {
  ...
  {
    "data": "Zebrafish LR Pair",
    "title": "<span title=\"Ligand-receptor Pair\">Zebrafish LR Pair</span>"
  },
  {
    "data": "Ligand Symbols",
  ...
  {
    "data": "Ligand ZFIN ID",
    "title": "<span title=\"Zebrafish Information Network ID (link to ZFIN)\">Ligand ZFIN ID</span>"
  },
  {
    "data": "Receptor ZFIN ID",
    "title": "<span title=\"Zebrafish Information Network ID (link to ZFIN)\">Receptor ZFIN ID</span>"
  },
  {
    "data": "Perplexity",
  ...
  {
    "data": "Zebrafish evidence",
    "title": "Zebrafish evidence"
  },
  {
  ...
  },
  {
    ...
    "title": "<span title=\"ENSEMBL gene ID for the receptor (link to ENSEMBL)\">Receptor ENSEMBL ID</span>"
        ...
        headerHtml += '</tr>';
        $('#zebrafish-table thead').html(headerHtml);
        ...
        var table = $('#zebrafish-table').DataTable({
            data: data,
            columns: [
                {
                    data: null,
        ...
        action: function(e, dt, node, config) {
            ...
            setTimeout(function() {
                ...
                {
                    extend: 'csvHtml5',
                    filename: 'ConnectomeDB2025_zebrafish',
                    exportOptions: {
            ...
            updateSelectionCounter(); // ADD THIS LINE
                ...
                setTimeout(function() {
                    ...
                    $('#zebrafish-table').closest('.dataTables_wrapper').prepend(filterContainer);

                    var filterDiv = $('<div style="max-width: 100%;"></div>');
                    var label = $('<label></label>');

                    // Create input without datalist (no autofill)
                    var input = $('<input type="text" placeholder="Search Ligand(s) / Receptor(s): i.e. ACE, CD24, EGFR, adam10b, zp3a.2" />')
                        .css({
                            'width': '100%',
                            'padding': '8px',
                            'font-size': '1.1rem',
...
api.on('order.dt search.dt', function () {
    ...
    if (searchVal && searchVal.trim() !== '') {
        ...
        console.error("JSON path:", "../../JSON/zebrafish_gene_pair.json");
