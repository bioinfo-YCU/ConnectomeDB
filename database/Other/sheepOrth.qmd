...
<div id="table-content-sheep" style="opacity: 0;">
...
<table id="sheep-table" class="display nowrap cell-border compact" style="width:100%">
    <thead></thead>
    <tfoot></tfoot>
</table>
...
$(document).ready(function() {
    ...
    $.getJSON("../../JSON/sheep_gene_pair.json", function(data) {
  ...
  {
    "data": "Sheep LR Pair",
    "title": "<span title=\"Ligand-receptor Pair\">Sheep LR Pair</span>"
  },
  {
    "data": "Ligand Symbols",
  ...
  },
  {
    ...
    "title": "Human Receptor Symbols"
        ...
        headerHtml += '</tr>';
        $('#sheep-table thead').html(headerHtml);
        ...
        var table = $('#sheep-table').DataTable({
            data: data,
            columns: [
                {
                    data: null,
        ...
        action: function(e, dt, node, config) {
            ...
            setTimeout(function() {
                ...
                {
                    extend: 'csvHtml5',
                    filename: 'ConnectomeDB2025_sheep',
                    exportOptions: {
            ...
            updateSelectionCounter(); // ADD THIS LINE
                ...
                setTimeout(function() {
                    ...
                    $('#sheep-table').closest('.dataTables_wrapper').prepend(filterContainer);

                    var filterDiv = $('<div style="max-width: 100%;"></div>');
                    var label = $('<label></label>');

                    // Create input without datalist (no autofill)
                    var input = $('<input type="text" placeholder="Search Ligand(s) / Receptor(s): i.e. ACE, CD24, EGFR, ENSOARG00020015296, IL10RA" />')
                        .css({
                            'width': '100%',
                            'padding': '8px',
                            'font-size': '1.1rem',
...
api.on('order.dt search.dt', function () {
    ...
    if (searchVal && searchVal.trim() !== '') {
        ...
        console.error("JSON path:", "../../JSON/sheep_gene_pair.json");
