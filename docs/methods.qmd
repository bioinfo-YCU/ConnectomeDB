...
For each literature-verified ligand–receptor pair, we recorded the species of origin for both ligand and receptor proteins. Most evidence came from human (2,511 pairs) and mouse (894 pairs), with additional support from rat (158 pairs) and other vertebrates (72 pairs). Cross-species experiments (where a ligand from one species was shown to bind to a receptor from another species) accounted for 1,082 pairs. Note that some pairs are supported by primarily literature in multiple species.

#### Orthology mapping
To enable analyses in other species, ligands and receptors in the original pairs were mapped to orthologues in human, mouse and 12 vertebrate species iincluding *Pan troglodytes* (chimp), *Macaca mulatta* (macaque), *Callithrix jacchus* (marmoset), *Equus caballus* (horse), *Sus scrofa* (pig), *Canis lupus* (dog), *Bos taurus* (cow), *Ovis aries* (sheep), *Rattus norvegicus* (rat), *Gallus gallus* (chicken), *Xenopus tropicalis* (frog) and *Danio rerio* (zebrafish). Data from the [Alliance of Genome Resources](https://www.alliancegenome.org/){target="_blank"} and [Ensembl](https://www.ensembl.org/index.html){target="_blank"} were used.
